...
class BiLSTM_Model(object):
    ...
    def __init__(self, entry="train"):
        self.model_config_path = os.path.join('models', 'BiLSTM_config.yml')
        self.model_param_path = os.path.join('models', 'BiLSTM_model_params.pkl')
    ...
    def save_model(self):
        try:
            ...
            print('BiLSTM model save success!')
        except Exception as error:
            print('BiLSTM model save failed! {}'.format(error))

    def restore_model(self):
        try:
            self.model.load_state_dict(torch.load(self.model_param_path))
            print("BiLSTM model restore success!")
        except Exception as error:
            print("BiLSTM model restore faild! {}".format(error))
        ...
        for epoch in range(30):
            ...
            for batch in self.train_manager.get_batch():  # yield生成器
                ...
                sentences_tensor = torch.tensor(sentences,
                                                dtype=torch.long)  # shape: torch.Size([batch_size, max_sentence_len])
                ...
                batch_size = sentences_tensor.shape[0]  # 例128

                # 一批数据的loss
                loss = self.model.neg_log_likelihood(sentences_tensor, tags_tensor, lengths_tensor)
                # 例loss=tensor([326.0190], grad_fn=<AddBackward0>)

                # 把loss转化为数字形式
                loss_to_num = loss.cpu().tolist()[0]  # 例loss_to_num=326.0190
    ...
    def evaluate(self, epoch=0):
        ...
        for batch in self.dev_manager.get_batch():  # yield生成器
            ...
            for unpadded_sentence, correct_path, predict_path in zip(unpadded_sentences, unpadded_labels,
                                                                     predict_paths):
...
if __name__ == '__main__':
    # bi_lstm_model = BiLSTM_Model(entry='train')
    # bi_lstm_model.train()
    bi_lstm_model = BiLSTM_Model(entry='test')
    bi_lstm_model.test()
